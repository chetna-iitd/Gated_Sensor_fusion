function [tot_energy_mcp,tot_energy_qin,tot_delay_mcp,tot_delay_qin,accuracy_mcp,accuracy_qin] = qin_motorway_exhaustive_constraint(delay,accuracy_con,num_users)
...
[tot_energy_qin(i,j),compute_energy_qin(i,j),communication_energy_qin(i,j),compute_delay_qin(i,j),communication_delay_qin(i,j),accuracy_qin(i,j)]=policy_gradient1_motorway(delay(1,i),accuracy_con(1,j),num_users);
[compute_energy1(i,j), compute_energy2(i,j), communication_energy1(i,j), communication_energy2(i,j), tot_energy1(i,j), tot_energy2(i,j),compute_delay_mcp(i,j),communication_delay_mcp(i,j),accuracy_mcp(i,j)]=weighted_fn1_motorway(delay(1,i),accuracy_con(1,j),num_users);
