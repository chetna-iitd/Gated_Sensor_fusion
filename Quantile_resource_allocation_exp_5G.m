function Quantile_resource_allocation_exp_5G()
...
tot_delay_mcp_user=zeros(17*(2^size(num_idx,1)),1);
tot_delay_qin_user=zeros(17*(2^size(num_idx,1)),1);
accuracy_mcp_user=zeros(17*(2^size(num_idx,1)),1);
accuracy_qin_user=zeros(17*(2^size(num_idx,1)),1);
user_count=zeros(17*(2^size(num_idx,1)),1);
...
accuracy_qin=zeros(num_users,1);
    ...
    tot_delay_mcp_user(counter:(counter+num_users-1),1)=tot_delay_mcp(1:num_users,1);
    tot_delay_qin_user(counter:(counter+num_users-1),1)=tot_delay_qin(1:num_users,1);
    accuracy_mcp_user(counter:(counter+num_users-1),1)=accuracy_mcp(1:num_users,1);
    accuracy_qin_user(counter:(counter+num_users-1),1)=accuracy_qin(1:num_users,1);
    user_count(counter:(counter+num_users-1),1)=num_users;
    for i=1:num_users
...
counter=counter+num_users;
end
counter=counter-num_users;
delay_user_qin=tot_delay_qin_user(1:counter-1);
delay_user_mcp=tot_delay_mcp_user(1:counter-1);
accuracy_user_mcp=accuracy_mcp_user(1:counter-1);
accuracy_user_qin=accuracy_qin_user(1:counter-1);
user_count_array=user_count(1:counter-1);
...
load('result.mat');
qq=0.1:0.1:1;
quantile_delay_mcp=quantile((delay_user_mcp./(user_count_array)),0.1:0.1:1);
quantile_delay_qin=quantile((delay_user_qin./(user_count_array)),0.1:0.1:1);
